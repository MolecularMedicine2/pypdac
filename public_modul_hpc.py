...
log_filename='./logs/{}_logfile.log'.format(temp_time)
logging.basicConfig(filename=log_filename, format='%(asctime)s %(levelname)s:%(message)s', level=logging.INFO)
if os.name == 'posix':
    os.system("chmod -R -v g+rwx {}".format(log_filename))
...
class Trainer():
    ...
    def train_model(self):
        ...
        with open('./logs/train_val_test_log_{}.csv'.format(self.datestr()), 'w', newline='') as csvfile:
            ...
            logging.info(('rejected samples', self.reject_counter))
            print('accepted samples', self.accept_counter)
            logging.info(('accepted samples', self.accept_counter))
            print('rate accepted ', self.accept_counter/(self.accept_counter+self.reject_counter + self.epsilon_fd))
            logging.info(('rate accepted ', self.accept_counter/(self.accept_counter+self.reject_counter + self.epsilon_fd)))
            ...
            with torch.no_grad():
                ...
                logging.info((self.args_printer(self.args)))
                logging.info((cm))
    ...
    def print_cuda(self):
        ...
        logging.info(('Active CUDA Device: GPU', torch.cuda.current_device()))
        print('Available devices ', torch.cuda.device_count())
        logging.info(('Available devices ', torch.cuda.device_count()))
        print('Current cuda device ', torch.cuda.current_device())
        logging.info(('Current cuda device ', torch.cuda.current_device()))
    ...
    def get_that_tiles(self, ):
            ...
            for TTC in self.train_tile_classes:
                ...
                if self.add_img:
                    if not (TTC == self.Not_TTC):#and TVT == 'train'
                        ...
                        images = []
                    images_add = glob.glob(os.path.join(self.add_img_path, TVT, TTC, '*.tif'))
        ...
        for clas in self.train_tile_classes:
            ...
            logging.info(('test Number of patches for {}:'.format(clas), len(d['test'][clas])))
            logging.info(('train Number of patches for {}:'.format(clas), len(d['train'][clas])))
            logging.info(('val Number of patches for {}:'.format(clas), len(d['val'][clas])))
            logging.info(('total Number of patches for {}:'.format(clas), len(d['test'][clas])+len(d['train'][clas])+len(d['val'][clas])))
...
##########################################################################################
    ...
    def calc_metrics_fl(self, y_scori, y_true, y_pred):
        ...
        if len(self.train_tile_classes) != 2:
            ...
            logging.info(('Only 2 classes no multiscore metrics'))
        return ret
